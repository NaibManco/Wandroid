...
public class LabelLayout extends ViewGroup implements View.OnClickListener {
    ...
    private void initAttr(AttributeSet attrs) {
        ...
        mLinePadding = t.getDimensionPixelSize(R.styleable.LabelLayout_line_padding, 0);
        childMargin = dip2px(t.getInt(R.styleable.LabelLayout_child_margin, 0));
        textBackground = t.getResourceId(R.styleable.LabelLayout_text_background, 0);
        t.recycle();
    }

    public void setLabelBackground(@IdRes int res) {
        textBackground = res;
    }
...
//            measureChildWithMargins(child, widthMeasureSpec, 0, heightMeasureSpec, 0); 这里置为0时候，和 measureChild 是一回事
    ...
    public void onClick(View v) {
        if (onItemClickListener != null) {
            int position = mChildView.indexOf(v);
            onItemClickListener.onItemClick(position, mAdapter.getItem(position));
        }
    }
